...
export class Niamh extends TelAdreAbstractContent implements TimeAwareInterface {
    ...
    public timeChange(player: Player): boolean {
...
//Approach:
export function approachNiamh(player: Player) {
    clearOutput();
    outputText(images.showImage("niamh-approach-in-bar"));
    if (Niamh.rand(5) == 0 && flags[kFLAGS.MET_NIAMH] > 0 && (player.hasItem(consumables.BIMBOLQ) || player.hasItem(consumables.SUCMILK))) {
        corruptOrBimboNiamhIntro(player);
        return;
    }
    ...
    //[Talk] [Get Beer] [Leave]
    simpleChoices("Talk", talkToNiamh, "Get Beer", beer, "", undefined, "", undefined, "Leave", leaveNiamh);
}
//[Get Beer]
export function getANiamhBeer(player: Player) {
    clearOutput();
    ...
    simpleChoices("Here", drinkNiamhsBeerInTelAdre, "To Go", togo, "", undefined, "", undefined, "", undefined);
}
//[Here]
function drinkNiamhsBeerInTelAdre(player: Player) {
    clearOutput();
    if (player.lactationQ() >= 300 && player.biggestTitSize() >= 5 && Niamh.rand(2) == 0 && flags[kFLAGS.MET_NIAMH] > 0) {
        ...
        simpleChoices("SellYourBooze", sellYourBooze, "", undefined, "", undefined, "", undefined, "Leave", leaveWithBeerTits);
        return;
    }
    ...
    blackCatBeerEffects(player, player, false, true);
    //both output
...
//[To Go]
function blackCatBeerToGo(player: Player) {
    clearOutput();
...
//Talk
function talkToNiamh(player: Player) {
    clearOutput();
    //(first time only)
    ...
    if (player.gems >= 2)
        beer = getANiamhBeer;
    simpleChoices("Beer", beer, "", undefined, "", undefined, "", undefined, "Leave", camp.returnToCampUseOneHour);
}
//Leave
function leaveNiamh() {
    clearOutput();
...
export function blackCatBeerEffects(player: Player, clearScreen: boolean = true, newLine: boolean = false) {
    if (clearScreen)
        clearOutput();
    if (newLine)
        outputText("\n\n");
    ...
    HPChange(40 + Niamh.rand(21), false);
    var lib: number = 0;
    if (player.findStatusAffect(StatusAffects.BlackCatBeer) >= 0) {
        ...
        dynStats("spe", -1, "int", -1, "lib", lib, "lus", 30 + Niamh.rand(player.lib / 4));
    }
    else {
        if (100 - player.lib >= 10)
            lib = 10;
        ...
        dynStats("spe", -5, "int", -5, "lib", lib, "lus", 20 + Niamh.rand(player.lib / 4));
    }
    player.slimeFeed();
}
//Black Cat Beer Wears Off: This message is displayed eight hours after the last drink.
export function blackCatBeerExpires(player: Player) {
...
function giveNiamphBimboLiquer(player: Player) {
    taintNiamh(player, true);
}
function giveNiamphSuccubiMilk(player: Player) {
    taintNiamh(player, false);
}
//Corruption Option
...
function corruptOrBimboNiamhIntro(player: Player) {
    clearOutput();
...
//[Maybe Later]
export function maybeLaterNiamh(player: Player) {
    clearOutput();
...
export function taintNiamh(player: Player, bimbo: boolean = false) {
    clearOutput();
...
export function niamhPostTelAdreMoveOut(player: Player) {
    ...
    simpleChoices("Yes", niamhCorruptedMobileSnackDrinkTime, "", undefined, "", undefined, "", undefined, "Leave", niamhCorruptMobileSnackTurnDown);
}
//[no]
function niamhCorruptMobileSnackTurnDown() {
    clearOutput();
...
//[yup]
function niamhCorruptedMobileSnackDrinkTime(player: Player) {
    clearOutput();
    ...
    if (flags[kFLAGS.NIAMH_STATUS] == 2) {
        ...
        bimboChampagne(player, player, false, false);
...
export function bimboChampagne(player: Player, clearScreen: boolean, intro: boolean) {
    if (clearScreen)
        clearOutput();
    if (intro) {
...
export function removeBimboChampagne(player: Player) {
...
export function bazaarNiamh(player: Player) {
    clearOutput();
    //Bimbo Niamh:
    if (flags[kFLAGS.NIAMH_STATUS] == 2) {
    ...
    if (player.hasCock())
        fuck = bazaarSex;
    simpleChoices("Get A Drink", drink, "Tit-Fuck", fuck, "", undefined, "", undefined, "Back", getGame().bazaar.enterTheBazaar);
}
//[bimbo/corrupted beer drink texts]
function bazaardNiamhDrink(player: Player) {
    clearOutput();
    player.gems -= 2;
    ...
    if (flags[kFLAGS.NIAMH_STATUS] == 2) {
        ...
        simpleChoices("Drink", drinkFromZeTap, "Bottle", getBimboChampFromNiamh, "", undefined, "", undefined, "", undefined);
    }
    //Corrupt
    else {
...
function drinkFromZeTap(player: Player) {
    clearOutput();
...
function getBimboChampFromNiamh() {
    clearOutput();
...
//[Bazaar sex]
function bazaarSex(player: Player) {
...
export function seanBimboBrewing(player: Player) {
    clearOutput();
    ...
    simpleChoices("Yep", yep, "", undefined, "", undefined, "", undefined, "Back", getGame().incubusShop);
}
//yeah I got this
function yeahSeanLetsBimbooze(player: Player) {
    clearOutput();
...
//24 hours later
export function getBimboozeFromSean() {
    clearOutput();
...
// [LEAVE]
function leaveWithBeerTits(player: Player) {
    clearOutput();
...
function sellYourBooze(player: Player) {
    clearOutput();
    player.gems += 2;
    temp = Math.round(player.lactationQ() / 100);
    if (temp > 40)
        temp = 40 + Niamh.rand(3);
    player.gems += temp;
    ...
    //If lust is high
        ...
        //[FUCK THE BAR] [LEAVE]
        simpleChoices("Fuck Bar", barBeerOrgyTits, "", undefined, "", undefined, "", undefined, "Leave", leaveWithBeerTits);
    }
}
...
//First Paragraph
//===============
function barBeerOrgyTits(player: Player) {
    clearOutput();
    //If [player has pussy]
    if (player.hasVagina())
    ...
    if ((player.pregnancyIncubation == 0 || player.pregnancyIncubation > 100) && player.bRows() > 1)
    ...
    if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 100)
    ...
    if (player.hasFuckableNipples())
        outputText("  Eventually some of the randier drunks start whipping out their cocks, and begin pressing the heads against the openings of your nipples.  Within moments you feel your booze bloated breasts being fucked silly by " + Niamh.num2Text(player.totalNipples()) + " hard cocks.  Each thrust causes " + boozeBoobsType() + " to spill out around their shafts.");
    ...
    if (player.cockTotal() > 0) {
        ...
        if (player.pregnancyIncubation > 0 && player.pregnancyIncubation < 200)
    ...
    if (player.statusAffectv1(StatusAffects.Edryn) >= 5 && player.gender == 3 && model.time.hours == 14 && flags[kFLAGS.URTA_COMFORTABLE_WITH_OWN_BODY] > 0) {
        ...
        // If [player has multiple cocks]
            ...
            outputText("  The heads of your cocks press in between Edryn's labia causing her to cry out, \"<i>Oh gawd!  How many dicks are y-you f-f-fucking meeee with???</i>\"  Edryn's orgasm cuts her voice off and every muscle in her cunt contracts around your " + multiCockDescriptLight() + ".");
    ...
    //Generic ending if the first two don't trigger
        ...
        if (player.cockTotal() == 1 && player.cockArea(0) < 75)
...
function boozeBoobsType() {
    ...
    // Christmas
        ...
        return "egg nog";
    // Valentine's
    else if (date.date == 14 && date.month == 1)
        return "wine";
    // Non-Holiday, Generic
    else
        return "beer";
